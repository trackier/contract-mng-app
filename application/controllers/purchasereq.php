...
{ 	
	...
	public function viewall() {	
		...
		if (!$uiQuery || ( $uiQuery && $uiQuery['status'] == '')) {
			$query['status'] = ['$in' => ['approved', 'rejected', 'processed', 'submitted', 'pending']];
		}
	...
	public function report() {	
		...
		if (!$uiQuery || ( $uiQuery && $uiQuery['status'] == '')) {
			$query['status'] = ['$in' => ['pending', 'approved', 'rejected', 'rejected', 'processed']];
		}
		$dq = ['start' => $this->request->get('start'), 'end' => $this->request->get('end')];
		$query['created'] = Db::dateQuery($dq['start'], $dq['end']);
	...
	public function add() {	
        ...
        if ($this->request->isPost()) {	
            ...
            $purchasereq->save();
			$user = User::userGchatMap($this->user->rm_id ?? '');
			$dataGchat = ['title' => 'Purchase Request Generated By Requester:'. $this->user->name, 'requester' => $this->user->name, 'amount' => $total,'pr_id' => $pr_id, 'prname' => $data['prname'], 'activity' => '', '_id' => $purchasereq->_id];
			$user = User::sendGoogleChatNotification($dataGchat, $user->gchatwebhook ?? '');
    ...
    public function approve1($id = null) {	
		...
		if ($id) {
			...
			$user = User::getFinanceHead();
			$requester = User::first(['_id' => $purchaseReqDetails->requester_id]);
			$dataGchat = ['title' => 'Purchase request approved by Department', 'requester' => $requester->name, 'amount' =>  $purchaseReqDetails->amount,'pr_id' => $purchaseReqDetails->pr_id, 'prname' => $purchaseReqDetails->prname, 'activity' => '', '_id' => $purchaseReqDetails->_id];
			$user = User::sendGoogleChatNotification($dataGchat, $user->gchatwebhook);
